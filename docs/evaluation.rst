...
.. note::
   ...
   **every** campaign folder (``<name>-<timestamp>``) found under the results directory.  The
...
- **run** -- executed once per individual run directory
  (``<name>-<timestamp>/<config>/<run-number>/``).
...
.. code-block:: python
   ...
   DATA_DIR = '/path/to/results/<name>-<timestamp>/<config-name>/'
