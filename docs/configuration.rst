...
- ``rosbags_to_csv``: Extract a specific set of ROS topics from rosbags to separate CSV files. Required ``topics`` parameter (list of topic names to extract). For each topic one CSV file per bag is written next to the bag, named ``<bag>_<topic>.csv``. Each scalar field of the message is a column. A field **declared** as an array of numbers — a ``LaserScan``'s ``ranges``, an ``Image``'s ``data``, a covariance — is a single column holding the whole array as ``num1:<dtype>:<count>:<base64 of the zlib-compressed little-endian values>``; read it back with ``robovast.results_processing.data.rosbags_common.decode_numeric_array``, or anywhere else with ``numpy.frombuffer(zlib.decompress(base64.b64decode(payload)), dtype='<' + dtype)``. Such a cell is wider than the per-cell limit of ordinary query results and comes back truncated there — the ``query.csv`` export (``csv_url``) returns it whole. A non-finite float, which is how a laser spells "no return", is written as an empty cell and ingested as ``NULL``, so one beam that hits nothing leaves the column numeric; the step names the columns it wrote a NULL in. A sequence of **sub-messages** (a ``Path``'s poses) is still a column per element per field, so a topic carrying many of them is one to reduce to scalars in the run rather than to record whole. For occupancy grids use ``rosbags_costmap_to_csv``, whose ``costmaps`` table the web :ref:`Run view <run-view>` reads and which keeps the grid's geometry beside its cells.
