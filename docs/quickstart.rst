...
.. code-block:: bash
   ...
   vast cluster setup --force    # re-provision, rebuilding the credential Secrets from .env
   vast cluster setup --force --rotate-token   # also mint a new access token (logs everyone out)
