...
class GlobalSettingsPage(WizardBasePage):
    ...
    def on_robot_diameter_changed(self, value):
        ...
        # Update variation data if available
        if self.variation_data and self.variation_data.general_parameters:
            self.variation_data.general_parameters.robot_diameter = value
    ...
    def _load_parameters_from_variation_data(self):
        """Load global parameters from VariationData into UI widgets."""
        if not self.variation_data or not self.variation_data.general_parameters:
            return
    ...
    def get_robot_diameter(self):
        """Get the current robot diameter setting."""
        return self.robot_diameter_spin.value()
    ...
    def validatePage(self):
        ...
        if self.variation_data and self.variation_data.general_parameters:
            self.variation_data.general_parameters.robot_diameter = (
                self.robot_diameter_spin.value()
            )
