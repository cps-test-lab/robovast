...
def main():
    ...
    # client-parameter
    ap.add_argument("--timeout-ms", type=int, required=True, help="Request timeout in milliseconds")
    ap.add_argument("--requests", type=int, required=True, help="Number of requests to send")
    ap.add_argument("--concurrency", type=int, required=True, help="Number of concurrent requests")
    ap.add_argument("--output", default="-", help="CSV-file or '-' für stdout")
    args = ap.parse_args()

    # No params to send - server is configured via its own command-line args
    params = {}
