...
class JobRunner:
    def __init__(self, variation_config, num_runs=1, single_variant=None):
        ...
        parameters = load_scenario_config(variation_config, subsection="execution")

        self.manifest_file_path = os.path.join(os.path.dirname(variation_config), parameters["kubernetes_manifest"])

        self.scenarios = self.get_execution_variants(variation_config)
    ...
    def get_execution_variants(self, variation_config):

        parameters = load_scenario_config(variation_config, subsection="execution")

        # Read filter patterns once
        scenario_filter_patterns = parameters.get("scenario_file_filter", [])
        variant_filter_patterns = parameters.get("variant_filter_patterns", [])

        if scenario_filter_patterns:
            print(f"### Loaded {len(scenario_filter_patterns)} filter patterns.")
        ...
        scenarios = self.get_filtered_files(variation_config, scenario_file, scenario_filter_patterns, variant_filter_patterns)
    ...
    def get_filtered_files(self, variation_file, scenario_file, scenario_filter_patterns, variant_filter_patterns):
        ...
        scenario_files = self.collect_filtered_files(scenario_filter_patterns, os.path.dirname(variation_file))
        ...
        for variant in variants:
            ...
            if "floorplan_variant_path" in variant:
                ...
                variant_files = self.collect_filtered_files(variant_filter_patterns, variant_file_path)
...
def main():
    ...
    parser.add_argument('--config', type=str, 
                        help='Config File containing the parameters for execution')
    ...
    try:
        job_runner = JobRunner(args.config, args.runs, args.variant)
        job_runner.run()
    except Exception as e:
        print(f"Error: {e}")
