...
def pull_workspace_to_directory(client, workspace_id: str, directory, *,
    ...
    """Fetch the workspace *workspace_id* into a local *directory*. Returns counts.

    The other direction of :func:`sync_directory_to_workspace`, over the workspace's
    archive: one request whatever the project holds, and the same bytes the share carries,
    so a project taken off a service and one taken off the share are the same tree.

    The archive's single top-level directory is stripped -- it names the workspace the
    export came from, and what the caller asked for is *directory*.

    The executable bit is carried back, because it is carried *out* by ``push_file`` and a
    run script that arrives non-executable fails at the point of use rather than here.
    ...
    # `r|gz` -- the stream is read once, forwards, so nothing has to hold the archive: the
    # chunks arrive off the socket and land as files.
    reader = _ChunkReader(client.workspace_tar_stream(workspace_id))
    with tarfile.open(fileobj=reader, mode="r|gz") as tar:
        for member in tar:
            ...
            if not member.isfile():
                # A workspace holds files; anything else came from somewhere that is not a
                # workspace, and a link would point into a tree this directory is not.
                continue
            target = _safe_target(target_root, rel)
            if target.exists() and not overwrite:
                raise FileExistsError(
                    f"{target} already exists. Pulling would overwrite a local copy that may "
                    f"hold edits this workspace does not have; pass overwrite to replace it.")
...
class _ChunkReader:
    """A minimal read-only file object over an iterator of byte chunks.

    ``tarfile`` in stream mode reads whatever size it likes, which an iterator of
    network-sized chunks cannot answer directly.
    """
