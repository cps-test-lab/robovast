...
# class MinimalS3ClusterConfig(BaseConfig):

#     def setup_cluster(self, **kwargs):
#         """Set up MinIO S3 server using emptyDir (ephemeral) storage.

#         Args:
#             **kwargs: Cluster-specific options (e.g. namespace)
#         """
#         logging.info("Setting up RoboVAST MinIO S3 server (minimal/emptyDir)...")

#         config.load_kube_config()
#         k8s_client = client.ApiClient()

#         namespace = kwargs.get('namespace', 'default')
#         manifests = inject_downloader_into_manifest(MINIO_MANIFEST_MINIMAL, namespace)
#         try:
#             apply_manifests(k8s_client, manifests, namespace=namespace)
...
#         """Clean up MinIO S3 server."""
#         logging.debug("Cleaning up RoboVAST MinIO (minimal_s3)...")
#         config.load_kube_config()
#         core_v1 = client.CoreV1Api()

#         namespace = kwargs.get('namespace', 'default')
#         manifests = inject_downloader_into_manifest(MINIO_MANIFEST_MINIMAL, namespace)
#         delete_manifests(core_v1, manifests, namespace=namespace)
#         logging.debug("MinIO manifest deleted successfully!")

#     def prepare_setup_cluster(self, output_dir, **kwargs):
#         """Write setup files for manual deployment.
...
#             f.write(MINIO_MANIFEST_MINIMAL)

#         readme_content = """# Minimal S3 Cluster Setup Instructions
...
# """
#         with open(f"{output_dir}/README_minimal_s3.md", "w") as f:
