...
def launch_controller(*, config_path, config_name, setup_kwargs, namespace,
    ...
    # Third-party variation plugins the campaign declares in the .vast's ``plugins:``
    # travel to the (otherwise bare) controller pod via a ``.robovast_plugins/`` dir
    # copied into the pod's project dir — the same layout the MCP/service flow uses.
    # Materialize them into a **temp** dir (not the user's source tree) with a full
    # install here, where any needed git credentials live; the pod imports them off
    # sys.path and never installs or clones. ``plugin_stage_dir`` is copied in and
    # cleaned up below.
    declared_plugins = getattr(campaign_config, "plugins", None)
    plugin_stage_dir = None
    if declared_plugins:
        from robovast.common.config_plugins import (  # pylint: disable=import-outside-toplevel
            PLUGIN_DIRNAME, ensure_workspace_plugins)
        click_echo("Preparing declared variation plugin(s) for the controller pod...")
        plugin_stage_dir = tempfile.mkdtemp(prefix="robovast_plugin_stage_")
        ensure_workspace_plugins(plugin_stage_dir, declared_plugins, force=True)
    ...
    try:
        ...
        _kubectl(ctx_args, "cp", config_dir, f"{namespace}/{pod_name}:{_POD_CAMPAIGN_DIR}",
                 "-c", "controller")
        # Copy the staged plugin dir into the pod's project dir (kept out of the
        # user's source tree, so it must be copied separately from config_dir).
        if plugin_stage_dir:
            staged = os.path.join(plugin_stage_dir, PLUGIN_DIRNAME)
            if os.path.isdir(staged):
                _kubectl(ctx_args, "cp", staged,
                         f"{namespace}/{pod_name}:{_POD_CAMPAIGN_DIR}/{PLUGIN_DIRNAME}",
                         "-c", "controller")
        ...
        # Reinstall the dev wheels over the image baseline with --no-deps so the
        # current robovast/robovast-nav source always wins. Third-party variation
        # plugins are not installed here — they are imported from the staged
        # ``.robovast_plugins/`` directory (see the note above).
        installs = []
        if pod_wheels:
            installs.append(
        ...
        if plugin_stage_dir:
            shutil.rmtree(plugin_stage_dir, ignore_errors=True)
