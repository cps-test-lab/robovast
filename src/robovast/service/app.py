...
def _read_mcp_calls(limit: int, tool: str, failed_only: bool, offset: int):
    ...
    return ([McpCall(at=c.at, tool=c.tool, duration_ms=c.duration_ms, ok=c.ok,
                     args=c.args, answer=c.answer, actor=c.actor,
                     session=c.session) for c in calls], total)
...
def build_app(impl: RobovastInterface, mount_mcp: bool = True,
    ...
    def export_mcp_calls(limit: int = 2000, tool: str = "", failed_only: bool = False,
        ...
        def rows():
            ...
            writer.writerow(["at", "tool", "duration_ms", "ok", "args", "answer",
                             "actor", "session"])
            for call in calls:
                writer.writerow([call.at, call.tool, round(call.duration_ms, 3), call.ok,
                                 call.args, call.answer, call.actor, call.session])
