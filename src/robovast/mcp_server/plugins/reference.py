...
def get_service_info() -> dict:
    """Which robovast-service is answering, which code it runs, and which backend it drives.
    ...
    **Check ``can_build_images`` before authoring a container that adds packages**, or the
    ...
    reason. It reports the infrastructure a build needs, not that a build will be published:
    a stale push credential reads ``true`` here and is refused by ``build_experiment_image``.
