...
class FloorplanVariation(NavVariation):
    ...
    @classmethod
    def collect_config_metadata(cls, config_entry: dict, config_dir: Path, campaign_dir: Path) -> dict:
        """Load map and mesh YAML metadata for this floorplan variation configuration.

        Reads the map file (``.yaml``) and mesh sidecar (``.stl.yaml``) that
        were generated by this variation and adds their contents to the metadata
        under the same key as the configuration parameter (e.g. ``map_file``,
        ``mesh_file``).  The generated FPM file is discovered from
        ``derived_from_files`` and recorded as ``fpm_file``; its ``.fpm.yaml``
        sidecar is loaded when present.
        """
        extra: dict = {}
        config_params = config_entry.get("config", {})
        if not isinstance(config_params, dict):
        ...
        # Retrieve derived_from_files recorded in the _variations entry for this class.
        # FPM entries (fpm/*.fpm) are separated out: they become fpm_file metadata,
        # while the remaining (json-ld/*) are used as derived_from for map/mesh.
        derived_from_files = []
        floorplan_file = {}
        for v in config_entry.get("variations", []):
            if v.get("name") == cls.__name__:
                raw_files = v.get("derived_from_files", [])

                fpm_rels = [rel for rel in raw_files if rel.endswith(".fpm")]
                non_fpm_rels = [rel for rel in raw_files if not rel.endswith(".fpm")]

                if fpm_rels:
                    fpm_campaign_path = f"{config_name}/_transient/{fpm_rels[0]}"
                    floorplan_file["file"] = fpm_campaign_path
                    fpm_yaml = campaign_dir / (fpm_campaign_path + ".yaml")
                    if fpm_yaml.exists():
                        try:
                            with open(fpm_yaml, "r") as f:
                                fpm_md = yaml.safe_load(f)
                                floorplan_file.update(**fpm_md)
                        except Exception as e:
                            logger.warning("Failed to load FPM metadata %s: %s", fpm_yaml, e)
                    extra["fpm_file"] = floorplan_file

                # Process FloorPlan's JSON-LD models (exclude fpm files from derived_from)
                derived_from_files = [
                    f"{config_name}/_transient/{rel}"
                    for rel in non_fpm_rels
                ]
                break
