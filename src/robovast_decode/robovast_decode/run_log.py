...
Timestamps are wall throughout -- sim time is added from a
:class:`~robovast_decode.clock_map.ClockMap`, and stays ``None`` where the map cannot answer
(see that module for why nothing is extrapolated).
"""
...
#:
#: A table is a set, and no engine owes anyone an insertion order, so a reader orders by
#: ``seq`` and never by where a row happens to be stored.
FIELDNAMES = [
    "seq",
...
def rosout_records(rows: Iterable[dict]) -> List[LogRecord]:
    """Pass 2: the ``rosout`` table's rows in the same record shape.
...
def collect_job_records(job_dir: str, rosout_rows: Iterable[dict] = (),
    ...
    *sole_container* names the one container this campaign runs, when it runs exactly one. A
    ...
    rosout = rosout_records(rosout_rows)
...
def rows_for_window(records: Sequence[LogRecord], clock, *,
    ...
    """Pass 4: the rows for one run -- sim time attached, window marked, ``seq`` numbered.

    *clock* is a :class:`~robovast_decode.clock_map.ClockMap`; where it cannot answer,
    ``sim_time`` is NULL. That happens for every line logged before the simulator
