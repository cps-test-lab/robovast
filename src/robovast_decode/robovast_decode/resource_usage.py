...
#: ``MAX`` is the only aggregate that does: a run's high-water mark is
#: ``MAX(shm_used_bytes)`` over its rows, bring-up included, and the limit in force is
#: ``MAX(shm_total_bytes)``.
FIELDNAMES = [
    ...
    for tick in ticks:
        ...
        for name, (cpu, mem, pids) in sorted(tick.processes.items()):
            rows.append({
                ...
                # tick, not to any one of them. NULL, never 0, where it was not measured.
                "shm_used_bytes": tick.shm_used_bytes,
...
def expected_container_files(containers: Optional[Sequence[str]]) -> Optional[Dict[str, str]]:
    ...
    *containers* are the campaign's runtime container names, the main one included, as its
    decoder configuration records them; ``None`` when it records none. The main container's
    file is named for its role (``resource_usage_main.csv``) while the container is called
    ``robovast`` everywhere a table names it, so the two are mapped here rather than at each
    reader.
    """
    if containers is None:
        return None
    return {name: (f"{_CSV_PREFIX}main.csv" if name == MAIN_CONTAINER
                   else f"{_CSV_PREFIX}{name}.csv") for name in containers}
