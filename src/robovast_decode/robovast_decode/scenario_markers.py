...
* :mod:`robovast_decode.derived`, which **records** it into ``scenario_timestamps`` from a
  run's merged log. Every later reader -- the web UI's playback clock and log views,
  ``search_run_logs`` -- reads that table rather than matching text again, so there is one
  answer to "when did the trial end".
