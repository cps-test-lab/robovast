...
# upstream_docs_args <repo-root>
# Sets UPSTREAM_DOCS_ARGS to the --build-context flag, or leaves it empty.
upstream_docs_args() {
  local root="$1"
  local out
  out=$(mktemp -d)
  ...
  do
    ...
    if [[ -d "$root/../$sibling/docs" ]]; then
      ...
      echo "upstream docs: $label from the checkout at ../$sibling ($(_docs_count "$out/$label") pages)"
      continue
    fi
    ...
    if [[ -z "$repo" || -z "$ref" ]]; then
      echo "upstream docs: no $repo_arg/$ref_arg in $dockerfile; $label will be missing" >&2
      continue
    fi
    tmp=$(mktemp -d)
    if git clone --quiet "$repo" "$tmp" 2>/dev/null && git -C "$tmp" checkout --quiet "$ref" 2>/dev/null; then
      _docs_copy "$tmp/docs" "$out/$label"
      echo "upstream docs: $label from $repo at ${ref:0:12} ($(_docs_count "$out/$label") pages)"
    else
      echo "upstream docs: could not fetch $repo at '$ref'; $label will be missing" >&2
    fi
    rm -rf "$tmp"
  done

  UPSTREAM_DOCS_ARGS=(--build-context "upstream-docs=$out")
}
