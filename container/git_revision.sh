# The revision a locally built image reports about itself.
#
# Sourced by container/robovast/build.sh and container/controller/build.sh so both bake the
# same value the same way -- for the reason platforms.env and buildcache.sh give, and with the
...
#
# Derived, never passed. There is deliberately no --revision option: an option is something to
# forget, and forgetting it is exactly the bug this fixes.
#
# The value must be byte-identical to what `robovast.common.execution._git_revision()` reports
...
# nothing, and `code_revision()` then reports "" -- "this deployment cannot tell you", which a
# caller can act on. A fabricated value would instead read as a revision that merely differs.

# git_revision_args [repo_root]
#
# Sets GIT_REVISION_ARGS to the --build-arg for that checkout's revision, or to nothing.
# Defaults to the repository this file lives in, which is the one whose code goes into the
# image -- not the caller's working directory, which may be a superproject holding robovast as
# a submodule.
git_revision_args() {
