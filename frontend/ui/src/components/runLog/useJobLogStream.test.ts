...
//
// `jobNameOf` is tested against the preview tree's own rows because nothing type-checks the
// agreement between the two ends: the service's job-log endpoint takes a `job_name` of
// `<config>/<run>`, and a drift on either side 404s at runtime with nothing pointing at why.

import { describe, expect, it } from 'vitest'
import { previewRunRows } from '@/lib/previewRuns'
