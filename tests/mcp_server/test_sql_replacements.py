...
A second campaign with the same configuration names sits beside the one under test, so a
query that reached it would answer about both in the same columns.
"""
...
from robovast.results_processing.data_query import (_MAX_CELL_BYTES, DataQueryError,
...
def campaign(tmp_path):
    ...
    that collide when a query spans both, so they are the only fixture on which a query
    reaching the other campaign shows up at all.
    """
    root = _write_batch_campaign(tmp_path / "camp-under-test", _SYSINFO)
    _write_batch_campaign(tmp_path / "camp-other", _OTHER_SYSINFO, speeds=(9.0, 9.5))
    return root
...
def _rows(campaign_dir, sql, campaigns=None):
    ...
    Spanning is explicit because a query sees one campaign's files: a query that reaches
    another campaign has to ask, which is what makes a cross-campaign read distinguishable
    from an accident.
    """
...
def test_run_view_does_not_bleed_across_campaigns(campaign):
    """The second bleed: the other campaign has the same two configuration names and the
    same run ids, so ``WHERE config_name='cfg-a' AND run_id=0`` would select a row in each
    -- rows indistinguishable in shape, so averaging over them silently mixes two campaigns.

    A query sees the campaign it was asked about, so it cannot span campaigns by omission.
    The fixture's collision is demonstrated the only way it can be -- by asking for both
    campaigns deliberately. If that returned one row the collision would not exist and this
...
def test_describe_documents_the_views_it_expects_callers_to_use(campaign):
    ...
    kinds = [t["kind"] for t in described["tables"]]
    assert kinds[:kinds.count("view")] == ["view"] * kinds.count("view")
    assert {"run_view", "config_view"} <= {t["table"] for t in described["tables"]
                                           if t["kind"] == "view"}
...
@pytest.fixture
def store_without(tmp_path, request):
    """A campaign whose ``campaign.db`` never had the tables named in ``request.param``.

    Alone in its scope, because the view's shape is decided from what the scope's stores
    hold: with another campaign's table present the LEFT JOIN — not the NULL literal — would
    be what answers. This fixture is the one that reaches the literal.
    """
...
def test_a_store_without_the_job_table_is_refused_naming_it(store_without):
    """The driver's store always has a ``job`` table; one without it is not a store this
    version reads, and a query says so rather than answering from part of it."""
    with pytest.raises(DataQueryError, match="job"):
        query_data_db(store_without, "SELECT config_name FROM run_view")
