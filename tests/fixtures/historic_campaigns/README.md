...
Four complete campaign directories, frozen at the config versions robovast has actually
...
| `v4-campaign-2026-09-07-090000` | 4 | a configuration's fixed values under `parameters:`, grouped by channel, plus a `configuration_presets:` block a configuration composes with `use:`. Declares both, so a later step meets the preset block and not only the entries |
