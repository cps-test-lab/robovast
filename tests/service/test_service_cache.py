...
"""Clearing the service's cache frees what nothing may still use, and says what it kept.

The cache is a copy of durable data -- worlds compiled from the scenes they were built
from -- so a clear costs only rebuild time. What makes it safe to offer as a button is
what it refuses to touch: an entry a viewer is loading right now. Each kept entry is
named with its reason, so "nothing to free" and "in use" read differently.
"""
