...
``NullService`` — the same client path the CLI and MCP tool take locally, so
...
def test_a_workspace_round_trips_through_a_directory(tmp_path):
    ...
    (source / "campaign.vast").write_text("version: 4\n", encoding="utf-8")
    ...
    assert (target / "campaign.vast").read_text(encoding="utf-8") == "version: 4\n"
