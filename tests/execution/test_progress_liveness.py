...
def test_the_cluster_backend_counts_the_same_files(tmp_path):
    ...
    Job is complete, so the cluster lane counts the files under the root. It inherits the
    count rather than restating it: the lane that answered ``None`` here ran every
    campaign with no batch total and a progress that never moved."""
