...
A campaign that dies mid-batch records no ``run`` rows and no run directories -- the only
thing SQL can still reach is what the campaign store itself recorded. Anything that lived
only in a run's output, or only in a JSON file, would therefore be unreachable by SQL for
exactly the campaign that needs explaining, and its post-mortem would have to be
reconstructed from a log after its pod was collected.
"""
...
def test_the_campaign_really_has_no_postprocessed_data(tmp_path):
    ...
    anything, because they would be reading data the happy path produced: no run
    directories, no metric files, no verdicts -- only the store."""
...
def test_forensics_are_queryable_when_the_campaign_never_postprocesses(tmp_path):
    ...
    That this answers at all is half the assertion: a campaign with no run directories and
    no metric files must still be queryable, or its post-mortem would be reachable only by
...
def test_a_failure_whose_runs_are_unresolved_still_appears(tmp_path):
    """Fanning out an empty run list yields no rows, so without the UNION ALL guard a failure
    the runner could not attribute to any run would vanish from the view entirely --
    silently, and in the case where something had already gone wrong enough to lose the
    mapping."""
...
def test_recording_an_incident_again_does_not_multiply_it(tmp_path):
    """The runner records the same incident again when it rereads the record; a post-mortem
    ...
    # `campaign.container_failure`, qualified: the campaign store's tables are the schema of
    # that name.
    result = query_data_db(tmp_path,
