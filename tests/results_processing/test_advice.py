...
def test_a_campaign_that_does_not_use_shared_memory_is_told_nothing(peak):
    ...
    nodes co-located in one process all touch almost none of it. A peak that fits in Docker's
    64Mi default needs no declaration or reduction, so there is nothing to say, including
    nothing about a declaration that is larger than it needs."""
...
def test_a_campaign_that_uses_the_pool_and_declares_nothing_is_warned():
    ...
    the node, and dies under Docker's 64Mi default -- of SIGBUS, not an OOM kill."""
