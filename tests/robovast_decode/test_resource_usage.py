...
def test_the_expected_set_comes_from_the_declared_containers():
    expected = resource_usage.expected_container_files(["robovast", "sut"])
...
def test_a_container_that_recorded_nothing_is_reported_not_inferred_away(tmp_path):
    ...
    resource_usage.collect_job_ticks(
        str(job), resource_usage.expected_container_files(["robovast", "sut"]),
        stats, "job-0")
...
def test_an_unexpected_csv_is_ingested_and_flagged(tmp_path):
    ...
    ticks = resource_usage.collect_job_ticks(
        str(job), resource_usage.expected_container_files(["robovast"]), stats, "job-0")
...
def test_a_tick_outside_the_clock_maps_range_keeps_its_wall_time(tmp_path):
    ...
    assert rows[0]["timestamp"] is None and rows[0]["wall_ts"] == 50.0
    assert rows[1]["timestamp"] == 50.0
