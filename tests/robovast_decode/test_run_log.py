...
def _rosout_rows(rows) -> list:
    """Rows of the ``rosout`` table, as the decoder builds it from ``/rosout``."""
    return [{"timestamp": recv, "stamp": stamp, "level": level_num, "level_name": level,
             "name": name, "msg": msg, "file": "f.cpp", "function": "fn", "line": 1}
...
def test_the_container_comes_from_the_filename():
    assert run_log.container_of("system.log") == MAIN_CONTAINER
...
def test_a_rosout_row_and_its_stdout_twin_become_one_row():
    ...
    merged = run_log.merge_records(
        ...
        run_log.rosout_records(rosout), stats)
    assert len(merged) == 1
    assert stats.matched == 1


def test_the_join_uses_the_producers_stamp_not_the_bags_receive_time():
    ...
    (rosout_rec,) = run_log.rosout_records(rosout)
...
def test_the_container_of_a_rosout_row_comes_from_its_twin():
...
def test_a_rosout_row_with_no_evidence_at_all_keeps_an_unknown_container():
    ...
    merged = run_log.merge_records([], run_log.rosout_records(rosout))
    assert merged[0].container == ""


def test_a_rosout_row_with_no_twin_takes_the_container_its_own_node_was_seen_in():
    ...
    stats = run_log.MergeStats()
    merged = run_log.merge_records(stdout, run_log.rosout_records(rosout), stats)
...
def test_a_node_seen_in_two_containers_stays_blank():
    ...
    rosout = _rosout_rows([(300.0002, 300.0, 20, "INFO", "entrypoint", "Uploading")])
...
def test_repeats_pair_in_time_order():
...
def test_the_same_words_at_a_different_time_stay_two_events():
    rosout = _rosout_rows([
...
def test_a_stamp_of_zero_falls_back_to_the_receive_time():
    ...
    rosout = _rosout_rows([(100.5, 0.0, 20, "INFO", "n", "early")])
    (rec,) = run_log.rosout_records(rosout)
    assert rec.wall_ts == 100.5


def test_rows_are_ordered_by_time():
    rosout = _rosout_rows([(102.0, 102.0, 20, "INFO", "n", "later")])
    merged = run_log.merge_records(
...
def test_seq_numbers_a_runs_rows_from_zero_in_merged_order():
    rows = run_log.rows_for_window(_one_record(100.0) + _one_record(100.0), NO_CLOCK_MAP)
...
#: The job's ``rosout`` table: one row no container's stdout carries.
_UNTWINNED = _rosout_rows([(200.0, 200.0, 20, "INFO", "recorder", "no twin anywhere")])
...
def test_a_single_container_campaign_attributes_every_row_to_it(tmp_path):
    ...
    records = run_log.collect_job_records(job, _UNTWINNED, sole_container="robovast")
...
def test_without_a_sole_container_an_untwinned_row_stays_unattributed(tmp_path):
    ...
    records = run_log.collect_job_records(job, _UNTWINNED)
...
def test_a_vanilla_sidecar_cannot_steal_another_containers_rows(tmp_path):
    ...
    records = run_log.collect_job_records(job, _UNTWINNED, sole_container=None)
    assert [r.container for r in records if r.message == "no twin anywhere"] == [""]
