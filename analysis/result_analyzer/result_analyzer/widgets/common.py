#!/usr/bin/env python3
"""
Common utilities for the GUI components
"""
...
def filter_nonrelevant_lines(log_content):
    """Filter out non-relevant lines from the log content."""
    log_content = "\n".join(
        line for line in log_content.splitlines()
        if "rviz2" not in line and "sys_stats_publisher" not in line
    )
    return log_content
