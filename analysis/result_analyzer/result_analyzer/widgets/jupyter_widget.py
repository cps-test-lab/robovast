"""
Enhanced Jupyter notebook data analysis widget for rosbag analysis.
Replaces QtConsole with web-based notebook execution for better performance and reliability.
"""
...
# Jupyter notebook execution imports
try:
    import nbformat
    ...
    print(f"Notebook execution not available: {e}")
...
class JupyterNotebookRunner(CancellableWorkload):
    ...
    @staticmethod
    def get_overview_paths(data_path, run_type):
        if run_type == RUN_TYPE.SINGLE_TEST:
            run_yaml_path = data_path / "run.yaml"
            if run_yaml_path.exists():
                return [str(data_path)]
        elif run_type == RUN_TYPE.SINGLE_VARIANT:
            run_yaml_paths = list(data_path.glob("*/run.yaml"))
            if run_yaml_paths:
                return [str(os.path.dirname(path)) for path in run_yaml_paths]
    ...
    @staticmethod
    def get_csvs(data_path, run_type):
    ...
    def get_notebook(self, path, run_type) -> str:
        ...
        # Match DATA_DIR in notebook JSON - may be inside quoted strings with escaped content
        regex_string = r'(?m)DATA_DIR\s*=\s*([\'"]).*?\1'
        replace_variable = "DATA_DIR"
        replace_string = f"'{os.path.abspath(path)}'"

        try:
            notebook_content, num_replacements = re.subn(
                regex_string,
                f"{replace_variable} = {replace_string}",
                notebook_content
            )
        except Exception as e:
            raise ValueError(f"Error during regex replacement: {e}")
    ...
    def run(self, data_path, run_type):
        ...
        try:
            ...
            notebook_content = self.get_notebook(data_path, run_type)
            if not notebook_content:
            ...
            class ProgressExecutePreprocessor(ExecutePreprocessor):
                def __init__(self, progress_callback=None, is_canceled_callback=None, *args, **kwargs):
                ...
                def preprocess_cell(self, cell, resources, index):
                    ...
                    if self.is_canceled_callback and self.is_canceled_callback():
                        raise Exception("Notebook execution canceled by user.")
