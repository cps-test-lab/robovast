#!/usr/bin/env python3
"""
Terminal Output Widget - A Qt widget for displaying terminal-like output with syntax highlighting
"""

import re

try:
    from PySide2.QtGui import QColor, QFont, QTextCharFormat, QTextCursor
    from PySide2.QtWidgets import (QHBoxLayout, QPushButton, QTextEdit,
                                   QVBoxLayout, QWidget)
    QT_SUPPORT = True
except ImportError:
    print("Error: PySide2 is required for terminal_output_widget.py")
    QT_SUPPORT = False
...
if __name__ == "__main__":
    """Test the terminal output widget"""
    import sys
    ...
    # Create main window
    window = QMainWindow()
    window.setWindowTitle("Terminal Output Widget Test")
    ...
    terminal_widget = TerminalOutputWidgetWithControls()
    ...
    def add_error():
        terminal_widget.append_output("This is an ERROR message")

    def add_warning():
        terminal_widget.append_output("This is a WARNING message")

    def add_success():
        terminal_widget.append_output("This operation was SUCCESS")

    def add_info():
        terminal_widget.append_output("This is an INFO message")

    error_btn = QPushButton("Add Error")
    error_btn.clicked.connect(add_error)
    test_layout.addWidget(error_btn)

    warning_btn = QPushButton("Add Warning")
    warning_btn.clicked.connect(add_warning)
    test_layout.addWidget(warning_btn)

    success_btn = QPushButton("Add Success")
    success_btn.clicked.connect(add_success)
    test_layout.addWidget(success_btn)

    info_btn = QPushButton("Add Info")
    info_btn.clicked.connect(add_info)
    ...
    print("Terminal output widget test window created.")
