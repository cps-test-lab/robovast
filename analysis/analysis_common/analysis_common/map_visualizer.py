...
class MapVisualizer:
    ...
    def create_figure(self, figsize: Tuple[int, int] = (12, 10)) -> Tuple[plt.Figure, plt.Axes]:
        """
    ...
    def draw_path(self, path_points: List[Tuple[float, float]],
                  ...
                  alpha: float = 0.8, label: str = 'Path') -> None:
        """
...
def load_and_display_map(yaml_path: str, figsize: Tuple[int, int] = (12, 10)) -> MapVisualizer:
    """
    Convenience function to load and display a map in one step.
    ...
    Returns:
        MapVisualizer instance with loaded map
    """
    viz = MapVisualizer()

    if viz.load_map(yaml_path):
        viz.create_figure(figsize)
        plt.show()
        return viz
    else:
        raise ValueError(f"Failed to load map from {yaml_path}")


if __name__ == "__main__":
    # Example usage
    print("Map Visualizer Module")
    print("Usage: import map_visualizer; viz = map_visualizer.load_and_display_map('path/to/map.yaml')")
